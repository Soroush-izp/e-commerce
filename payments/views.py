    ...
    def post(self, request, *args, **kwargs):
        try:
            ...
            if order.status not in ["pending", "failed"]:
                return Response(
                    {"order": "Order is not in a valid state for payment."},
            ...
            if payment:
                ...
                payment.authority = (
                    None  # Clear the authority for a new payment request
                )
                payment.save()
            else:
            ...
            if result["errors"]:
                raise ValidationError(result["errors"]["message"])
            ...
            raise ValidationError("Unable to process payment request")
    ...
    def get(self, request, *args, **kwargs):
        ...
        try:
            ...
            if response_data.get("data", {}).get("code") == 100:
                ...
                return Response(
                    self.serializer_class(payment).data, status=status.HTTP_200_OK
                )
            else:
                ...
                return Response(
                    {
                        ...
                        "details": response_data.get("data", {}).get(
            ...
            return Response(
                {"error": "Failed to connect to payment gateway", "details": str(e)},
...
class PaymentHistoryView(ListAPIView):
    ...
    def get_queryset(self):
        return PaymentDetails.objects.filter(user=self.request.user).order_by(
            "-created_at"
        )
