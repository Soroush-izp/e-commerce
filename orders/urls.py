...
urlpatterns = [
    ...
    path(
        "wishlists/",
        AuthenticatedUserWishlistListView.as_view(),
        name="user-wishlist-list",
    ),
    # User: Retrieve (GET), update (PUT, PATCH), or delete (DELETE) a specific wishlist item by ID
    path(
        "wishlists/<int:pk>/",
        AuthenticatedUserWishlistDetailView.as_view(),
        name="user-wishlist-detail",
    ),
    # Wishlist APIs for admin or superuser
    # Admin: Retrieve a list of all wishlists (GET)
    path(
        "admin/wishlists/", AdminWishlistListView.as_view(), name="admin-wishlist-list"
    ),
    # Admin: Retrieve (GET), update (PUT, PATCH), or delete (DELETE) a specific wishlist by ID
    path(
        "admin/wishlists/<int:pk>/",
        AdminWishlistDetailView.as_view(),
        name="admin-wishlist-detail",
    ),
    ...
    path(
        "user/orders/", UserOrderListCreateView.as_view(), name="user-order-list-create"
    ),
    # User: Retrieve (GET), update (PUT, PATCH), or delete (DELETE) a specific order by ID
    path(
        "user/orders/<int:pk>/", UserOrderDetailView.as_view(), name="user-order-detail"
    ),
    # User: List all items for a specific order of the authenticated user (GET)
    path(
        "user/orders/<int:order_id>/items/",
        UserOrderItemListView.as_view(),
        name="user-order-item-list",
    ),
]
