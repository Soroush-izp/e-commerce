    ...
    def __init__(self, *args, **kwargs):    
        ...
        if request is not None:
            if request.method == 'GET':
                ...
                allowed_fields = ['id', 'brand_name', 'icon', 'is_active']  # Define allowed fields
...
class BrandDetailSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = Brand
        fields = ['id', 'brand_name', 'description', 'website', 'instagram', 'facebook', 'icon', 'is_active', 'photos', 'videos']
...
class AttributeGroupSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        fields = ['id', 'name', 'attributes', 'created_at', 'is_active', 'attribute_details']
...
class AdminCategorySerializer(serializers.ModelSerializer):
    all_attribute_groups = AttributeGroupSerializer(many=True, read_only=True)  # For combined parent and category groups
    created_at = serializers.DateTimeField(read_only=True)  # Make it read-only
    level = serializers.IntegerField(read_only=True)  # Make level read-only

    class Meta:
        model = Category
        fields = ['id', 'name', 'parent', 'attribute_groups', 'all_attribute_groups', 'photo', 'description', 'level', 'created_at', 'is_active']

    def to_representation(self, instance):
        """Override to include parent attribute groups combined with the category's own."""
        representation = super().to_representation(instance)
        representation['all_attribute_groups'] = AttributeGroupSerializer(instance.get_all_attribute_groups(), many=True).data
...
class UserCategoryDetailSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = Category
        fields = [
            'id', 'name', 'parent', 'photo', 'description', 'level', 
            'is_active', 'all_attribute_groups', 'subcategories', 'parent_category'
        ]
...
class ProductListSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = Product
        fields = ['id', 'name', 'cover', 'category', 'attribute_groups', 'price_range', 'is_available', 'created_at', 'is_active']

    def get_attribute_groups(self, obj) -> list[dict]:
        return [{'id': group.id, 'name': group.name} for group in obj.category.get_all_attribute_groups()]
...
class ProductSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = Product
        fields = ['id', 'name', 'description', 'summary', 'cover', 'category', 'attribute_groups', 'price_range', 'is_available', 'created_at', 'is_active']
...
class ProductDetailSerializer(serializers.ModelSerializer):
    ...
    def validate(self, data):
        ...
        if ProductDetail.objects.filter(product=product, order_num=order_num).exists():
            raise serializers.ValidationError("Order number must be unique per product.")
        return data
...
class ProductDetailBatchUpdateSerializer(serializers.Serializer):
    ...
    def update(self, instance, validated_data):
        ...
        if not update_ids.issubset(existing_ids):
            raise serializers.ValidationError("Some ProductDetail IDs do not belong to the specified product.")

        with transaction.atomic():
            # Step 1: Delete any ProductDetail records that are not in the update list
            ProductDetail.objects.filter(product_id=product_id).exclude(id__in=update_ids).delete()
            ...
            temp_order_num = max(
                ProductDetail.objects.filter(product_id=product_id).values_list('order_num', flat=True)
            ) + 1
            ...
            for update_item in update_data:
                ...
                temp_order_num += 1
                ProductDetail.objects.filter(id=detail_id).update(order_num=temp_order_num)
...
class ProductSKUAttributeSerializer(serializers.ModelSerializer):
    ...
    show_attribute_values = ProductAttributeValueSerializer(source='attribute_value', read_only=True)  # Use the existing attribute_value as source
...
class ProductSKUSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = ProductSKU
        fields = ['id', 'sku', 'product', 'price', 'quantity', 'sku_attributes',  'created_at', 'is_active']
...
class ReviewSectionSerializer(serializers.ModelSerializer):
    texts = serializers.PrimaryKeyRelatedField(many=True, read_only=True)  # Displays list of ReviewText IDs associated with this section
    photos = serializers.PrimaryKeyRelatedField(many=True, read_only=True)  # Displays list of ReviewPhoto IDs associated with this section
    videos = serializers.PrimaryKeyRelatedField(many=True, read_only=True)  # Displays list of ReviewVideo IDs associated with this section

    class Meta:
        model = ReviewSection
        fields = ['id', 'product', 'title', 'order_num', 'texts', 'photos', 'videos']  # Includes foreign key product, title, and related fields
        read_only_fields = ['order_num']  # Ensures order_num is read-only as it is auto-generated


# Serializer for ReviewText model
class ReviewTextSerializer(serializers.ModelSerializer):
    review_section = serializers.PrimaryKeyRelatedField(queryset=ReviewSection.objects.all())  # Allows linking to a ReviewSection

    class Meta:
        model = ReviewText
        fields = ['id', 'review_section', 'text', 'order_num']  # Includes foreign key review_section and review text
...
class ReviewPhotoSerializer(serializers.ModelSerializer):
    review_section = serializers.PrimaryKeyRelatedField(queryset=ReviewSection.objects.all())  # Allows linking to a ReviewSection
    position = serializers.ChoiceField(choices=ReviewPhoto.POSITION_CHOICES)  # Allows selection from predefined choices

    class Meta:
        model = ReviewPhoto
        fields = ['id', 'review_section', 'image', 'position', 'order_num']  # Includes foreign key review_section, image, position, and order number
        read_only_fields = ['order_num']  # Ensures order_num is read-only
...
class ReviewVideoSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = ReviewVideo
        fields = ['id', 'review_section', 'video', 'order_num']  # Includes foreign key review_section, video file, and order number
        read_only_fields = ['order_num']  # Ensures order_num is read-only
...
class ReviewSectionDetailSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = ReviewSection
        fields = [
            'id', 'product', 'title', 'order_num', 
            'texts', 'photos', 'videos',  # ID-only references
            'items'
        ]
...
class SwapOrderNumSerializer(serializers.Serializer):
    ...
    def validate(self, data):
        ...
        if data['id1'] == data['id2']:
            raise serializers.ValidationError("The IDs must be different to swap order numbers.")
        return data
...
class SwapOrderNumItemsSerializer(serializers.Serializer):
    ...
    def validate(self, data):
        ...
        if not self._order_num_exists(review_section_id, order_num_1):
            raise serializers.ValidationError(f"Order number {order_num_1} not found in ReviewSection.")
        if not self._order_num_exists(review_section_id, order_num_2):
            raise serializers.ValidationError(f"Order number {order_num_2} not found in ReviewSection.")
    ...
    def _order_num_exists(self, review_section_id, order_num):
        ...
        return (
            ReviewText.objects.filter(review_section_id=review_section_id, order_num=order_num).exists() or
            ReviewPhoto.objects.filter(review_section_id=review_section_id, order_num=order_num).exists() or
            ReviewVideo.objects.filter(review_section_id=review_section_id, order_num=order_num).exists()
        )
