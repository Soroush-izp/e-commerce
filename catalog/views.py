...
class ProductAttributeValueListView(generics.ListAPIView):
    serializer_class = ProductAttributeValueSerializer
    permission_classes = [IsAuthenticated, IsAdminUser | IsSuperUser]  # Adjust as necessary

    def get_queryset(self):
        attribute_type_id = self.kwargs.get('attribute_type_id')
        return ProductAttributeValue.objects.filter(type_id=attribute_type_id, is_active=True)
...
class AdminProductListManageView(generics.GenericAPIView):
    ...
    serializer_class = ProductListSerializer
    permission_classes = [IsAuthenticated, IsAdminUser | IsSuperUser]  # Only accessible to authenticated admins
...
class ProductDetailSwapOrderView(generics.GenericAPIView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        with transaction.atomic():
            temp_order_num = max(   # biggest order_num + 1
                ProductDetail.objects.filter(product_id=first_detail.product_id)
...
class AdminProductPhotoDetailView(generics.RetrieveUpdateDestroyAPIView):
    ...
    def get_object(self):
        ...
        try:
            ...
            raise Response({"error": "Product photo not found."}, status=status.HTTP_404_NOT_FOUND)
...
class AdminProductVideoDetailView(generics.RetrieveUpdateDestroyAPIView):
    ...
    def get_object(self):
        ...
        try:
            ...
            raise Response({"error": "Product video not found."}, status=status.HTTP_404_NOT_FOUND)
...
class ProductSKUAttributeListCreateView(generics.ListCreateAPIView):
    ...
    def get_permissions(self):
        if self.request.method == 'POST':
            ...
            return [IsAuthenticated(), IsAdminUser()]  # Instantiate each permission class
        # Allow any user to view the SKU list
        return [AllowAny()]
...
class SwapOrderNumView(generics.GenericAPIView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        if serializer.is_valid():
            ...
            with transaction.atomic():
                temp_order_num = max(
                    ReviewSection.objects.filter(product_id=obj1.product_id)
                    .values_list('order_num', flat=True)
                ) + 1
            ...
            return Response(
                {"message": "Order numbers swapped successfully", "obj1": obj1_data, "obj2": obj2_data},
                status=status.HTTP_200_OK
            )
...
class SwapOrderNumItemView(generics.GenericAPIView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        with transaction.atomic():
            ...
            self._swap_order(ReviewText, review_section_id, order_num_1, order_num_2, temp_order_num)
            self._swap_order(ReviewPhoto, review_section_id, order_num_1, order_num_2, temp_order_num)
            self._swap_order(ReviewVideo, review_section_id, order_num_1, order_num_2, temp_order_num)

        return Response({"message": "Order numbers swapped successfully."}, status=status.HTTP_200_OK)

    def _get_max_order_num(self, review_section_id):
        """
        Get the maximum order_num across all models for the given ReviewSection.
        """
        max_text = ReviewText.objects.filter(review_section_id=review_section_id).aggregate(models.Max('order_num'))['order_num__max'] or 0
        max_photo = ReviewPhoto.objects.filter(review_section_id=review_section_id).aggregate(models.Max('order_num'))['order_num__max'] or 0
        max_video = ReviewVideo.objects.filter(review_section_id=review_section_id).aggregate(models.Max('order_num'))['order_num__max'] or 0

        return max(max_text, max_photo, max_video)

    def _swap_order(self, model, review_section_id, order_num_1, order_num_2, temp_order_num):
        """
        Swap order_num for a given model within the same ReviewSection.
        """
        # Step 1: Move order_num_1 to a temporary value
        model.objects.filter(review_section_id=review_section_id, order_num=order_num_1).update(order_num=temp_order_num)

        # Step 2: Move order_num_2 to order_num_1
        model.objects.filter(review_section_id=review_section_id, order_num=order_num_2).update(order_num=order_num_1)

        # Step 3: Move the temporary value to order_num_2
        model.objects.filter(review_section_id=review_section_id, order_num=temp_order_num).update(order_num=order_num_2)
